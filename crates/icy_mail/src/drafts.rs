...
impl DraftStore {
    ...
    pub fn issues(&self, draft: &Draft) -> Vec<DraftIssue> {
        ...
        for (field, value) in [
            ...
            character_issues(field, value, &mut issues);
        }
        ...
        if body.trim().is_empty() {
            ...
            for line in body.split('\n') {
                character_issues(DraftField::Body, line, &mut issues);
            }
        }
        issues
    }
...
fn character_issues(field: DraftField, value: &str, issues: &mut Vec<DraftIssue>) {
