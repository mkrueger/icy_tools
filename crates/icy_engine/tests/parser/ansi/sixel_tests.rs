...
use crate::{
    Position, TextAttribute, TextScreen, ansi::Parser, parsers::{create_buffer, update_buffer}, screen::Screen
};

fn update_sixels(buf: &mut TextScreen) {
    while !buf.buffer.sixel_threads.is_empty() {
        buf.buffer.update_sixel_threads().unwrap();
...
fn test_simple_sixel() {
    let mut buf = create_buffer(
        &mut Parser::default(),
        b"\x1BPq#0;2;0;0;0#1;2;100;100;0#2;2;0;100;0#1~~@@vv@@~~@@~~$43#2??}}GG}}??}}??-#1!14@\x1B\\",
    );
    update_sixels(&mut buf);
    assert_eq!(1, buf.buffer.layers[0].sixels.len());
    assert_eq!(2, buf.buffer.layers[0].sixels[0].vertical_scale);
    assert_eq!(1, buf.buffer.layers[0].sixels[0].horizontal_scale);
    assert_eq!(Position::new(0, 0), buf.buffer.layers[0].sixels[0].position);
    assert_eq!(14, buf.buffer.layers[0].sixels[0].get_width());
    assert_eq!(12, buf.buffer.layers[0].sixels[0].get_height());
}
...
fn test_simple_position_sixel() {
    ...
    update_sixels(&mut buf);
    let sixels = &buf.buffer.layers[0].sixels;
...
fn test_macro_sixels() {
    ...
    update_buffer(&mut buf, &mut parser, b"\x1B[10;59f\x1BP\x1B[11*z\x1B\\");
    update_sixels(&mut buf);
    {
    assert_eq!(1, buf.buffer.layers[0].sixels.len());
    }
    update_buffer(&mut buf, &mut parser, b"\x1B[0;59f\x1BP\x1B[11*z\x1B\\");
    update_sixels(&mut buf);

    {
        assert_eq!(2, buf.buffer.layers[0].sixels.len());
    }
}
...
fn test_simple_sixel2() {
    let mut parser: Parser = Parser::default();
    let mut buf = create_buffer(&mut parser, b"\x1B[1;1H");
    update_sixels(&mut buf);
...
fn test_sixel_raster_attributes() {
    let mut buf = create_buffer(&mut Parser::default(), b"\x1BPq\"2;3;6;8\x1B\\");
    update_sixels(&mut buf);

    assert_eq!(1, buf.buffer.layers[0].sixels.len());
    assert_eq!(3, buf.buffer.layers[0].sixels[0].horizontal_scale);
    assert_eq!(8, buf.buffer.layers[0].sixels[0].get_height());
    assert_eq!(6, buf.buffer.layers[0].sixels[0].get_width());
...
fn test_sixel_removal() {
    ...
    buf.buffer.layers[0].set_char((0, 0), crate::AttributedChar::new(' ', TextAttribute::default()));

    assert_eq!(0, buf.buffer.layers[0].sixels.len());
}
