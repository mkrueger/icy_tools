app-about = Przeglądarka dla sztuki ANSI, sztuki ASCII i innych dzieł tekstowych

# Argumenty wiersza poleceń
arg-path-help = Ścieżka do pliku lub katalogu do otwarcia
arg-auto-help = Włącz animację automatycznego przewijania
arg-bps-help = Emulacja szybkości transmisji (np. 9600, 19200, 38400)
heading-title=Tytuł
heading-author=Autor
heading-group=Grupa
heading-screen-mode=Flagi

menu-item-discuss=Dyskutuj
menu-item-report-bug=Zgłoś błąd
menu-item-check-releases=Najnowsza wersja
menu-item-auto-scroll=Automatyczne przewijanie
menu-item-scroll-speed-slow=Wolna prędkość
menu-item-scroll-speed-medium=Średnia prędkość
menu-item-scroll-speed-fast=Szybka prędkość
menu-item-set_terminal_width=Ustaw szerokość terminala
menu-item-settings=Ustawienia

menu-upgrade_version=Uaktualnij do { $version }

update-available=Dostępna aktualizacja: { $version }

tooltip-refresh=Odśwież
tooltip-reset-filter-button=Resetuj filtr
tooltip-back=Wstecz
tooltip-forward=Dalej
tooltip-up=Przejdź do katalogu nadrzędnego
tooltip-filter=Filtruj (Ctrl+F)
tooltip-view-mode-list=Przełącz na widok listy
tooltip-view-mode-tiles=Przełącz na widok kafelków
tooltip-browse-16colors=Przeglądaj 16colors.rs
tooltip-settings=Ustawienia
tooltip-sort-name-asc=Sortuj według nazwy (A-Z)
tooltip-sort-name-desc=Sortuj według nazwy (Z-A)
tooltip-sort-size-asc=Sortuj według rozmiaru (najmniejsze najpierw)
tooltip-sort-size-desc=Sortuj według rozmiaru (największe najpierw)
tooltip-sort-date-asc=Sortuj według daty (najstarsze najpierw)
tooltip-sort-date-desc=Sortuj według daty (najnowsze najpierw)
tooltip-sauce-mode-on=Pokaż informacje SAUCE
tooltip-sauce-mode-off=Ukryj informacje SAUCE
tooltip-shuffle-mode=Tryb losowy (pokaz slajdów)

header-name=Nazwa
header-title=Tytuł
header-author=Autor
header-group=Grupa

statusbar-items={ $count } elementów
statusbar-ready=Gotowy

filter-entries-hint-text=Filtruj wpisy
label-terminal_width=Szerokość terminala:

message-loading-image=Ładowanie obrazu…
message-file-not-supported=Plik { $name } może nie być obsługiwany.
button-load-anyways=Załaduj mimo to
button-ok=OK
button-cancel=Anuluj
button-open=Otwórz
message-empty=Nie zobaczysz tutaj nic, dopóki nie wybierzesz obsługiwanego pliku.

error-invalid-path=Nieprawidłowa ścieżka
error-never-happens=To nie powinno się zdarzyć :) - zgłoś błąd!

sauce-dialog-title=Informacje SAUCE
sauce-dialog-title-label=Tytuł:
sauce-dialog-author-label=Autor:
sauce-dialog-group-label=Grupa:
sauce-dialog-comments-label=Komentarze:
sauce-dialog-date-label=Data:
sauce-dialog-flags-label=Flagi:
sauce-dialog-font-name=Czcionka:

sauce-unknown=Nieznany
sauce-btn-formatted=Sformatowane
sauce-btn-raw=Surowe

sauce-section-info=Informacje SAUCE
sauce-section-capabilities=Możliwości
sauce-section-comments=Komentarze
sauce-section-raw-header=Surowy Nagłówek SAUCE
sauce-section-technical=Informacje Techniczne
sauce-section-comment-lines=Linie Komentarzy

sauce-field-title=Tytuł
sauce-field-author=Autor
sauce-field-group=Grupa
sauce-field-date=Data
sauce-field-type=Typ
sauce-field-file-size=Rozmiar Pliku
sauce-field-format=Format
sauce-field-size=Rozmiar
sauce-field-flags=Flagi
sauce-field-columns=Kolumny
sauce-field-lines=Linie
sauce-field-ice-colors=Kolory iCE
sauce-field-letter-spacing=9px Font
sauce-field-aspect-ratio=Proporcje
sauce-field-font=Czcionka
sauce-field-width=Szerokość
sauce-field-height=Wysokość
sauce-field-pixel-depth=Głębokość Pikseli
sauce-field-sample-rate=Częstotliwość Próbkowania
sauce-field-data-type=DataType
sauce-field-file-type=FileType
sauce-field-tinfo1=TInfo1
sauce-field-tinfo2=TInfo2
sauce-field-tinfo3=TInfo3
sauce-field-tinfo4=TInfo4
sauce-field-tflags=TFlags
sauce-field-tinfos=TInfoS

sauce-value-yes=Tak
sauce-value-8px=8px
sauce-value-9px=9px
sauce-value-legacy=Starszy
sauce-value-modern=Square
sauce-value-none=Brak
sauce-value-executable=Wykonywalny
sauce-value-bytes={ $count } bajtów
sauce-value-lines={ $count } linii
sauce-value-pixels={ $count }px
sauce-value-bpp={ $count }bpp
sauce-value-hz={ $count } Hz

# Okno pomocy
help-title = Skróty klawiszowe
help-subtitle = Szybka pomoc dla iCY VIEW

toast-auto-scroll-on=Automatyczne przewijanie włączone
toast-auto-scroll-off=Automatyczne przewijanie wyłączone
toast-scroll-slow=Prędkość przewijania: wolna
toast-scroll-medium=Prędkość przewijania: średnia
toast-scroll-fast=Prędkość przewijania: szybka

button-play_music=Odtwórz muzykę
button-stop_music=Zatrzymaj muzykę

label-music_pause=Pauza { $duration }ms
label-music_note=Odtwórz { $note }({ $octave }) przez { $duration }ms

label-sixteencolors_year= { $year } ({ $packs } paczki)

settings-heading=Ustawienia
settings-reset_button=Resetuj
settings-monitor-category=Monitor
settings-paths-category=Ścieżki

settings-paths-header=Ścieżki aplikacji
settings-paths-user-header=Ścieżki użytkownika
settings-paths-export-path=Ścieżka eksportu:
settings-paths-config-dir=Katalog konfiguracji:
settings-paths-config-file=Plik konfiguracji:
settings-paths-log-file=Plik dziennika:
settings-paths-open=Otwórz

settings-commands-category=Polecenia
settings-commands-section=Programy Zewnętrzne
settings-commands-placeholder=Polecenie (np. icy_draw %F)
settings-commands-description=Użyj %F dla nazwy pliku

export-no-file-selected=Nie wybrano pliku do eksportu
export-no-screen-available=Brak dostępnego ekranu do eksportu
export-success=Wyeksportowano do { $path }

toast-baud-rate-off=Emulacja baud: wyłączona
toast-baud-rate=Szybkość transmisji: { $rate }
toast-command-not-configured=Polecenie { $key } nie skonfigurowane

preview-no-file-selected = Nie wybrano pliku
preview-loading = Ładowanie...
preview-error = Błąd: { $message }
preview-error-title = Nie udało się załadować pliku

error-read-file-data = Nie można odczytać zawartości pliku
error-read-file = { $error }

error-external-command-title = Nie udało się wykonać polecenia zewnętrznego
error-external-command-message = Polecenie: { $command }

    Błąd: { $error }

welcome-select-file = 📂 Wybierz plik do podglądu
welcome-tip = Wskazówka: Naciśnij Ctrl+F aby filtrować, lub kliknij 🌐 aby przeglądać 16colors.rs

thumbnail-loading = Ładowanie...
thumbnail-no-diz = brak file_id.diz
thumbnail-unsupported = Nieobsługiwany

filter-no-items-found = Nie znaleziono elementów.
folder-empty = Folder jest pusty.

# ═══════════════════════════════════════════════════════════════════════════════
# System poleceń
# ═══════════════════════════════════════════════════════════════════════════════

# Kategorie
cmd-category-file = Plik
cmd-category-edit = Edycja
cmd-category-view = Widok
cmd-category-navigation = Nawigacja
cmd-category-window = Okno
cmd-category-help = Pomoc
cmd-category-settings = Ustawienia
cmd-category-dialog = Dialogi
cmd-category-playback = Odtwarzanie
cmd-category-external = Zewnętrzne

# Polecenia pliku
cmd-file-open-action = Otwórz
cmd-file-open-desc = Otwórz plik
cmd-file-export-action = Eksportuj
cmd-file-export-desc = Eksportuj jako obraz
cmd-file-close-action = Zamknij
cmd-file-close-desc = Zamknij aktualny plik

# Polecenia edycji
cmd-edit-copy-action = Kopiuj
cmd-edit-copy-desc = Kopiuj zaznaczenie
cmd-edit-paste-action = Wklej
cmd-edit-paste-desc = Wklej ze schowka
cmd-edit-select_all-action = Zaznacz wszystko
cmd-edit-select_all-desc = Zaznacz całą zawartość

# Polecenia widoku
cmd-view-zoom_in-action = Powiększ
cmd-view-zoom_in-desc = Zwiększ powiększenie
cmd-view-zoom_out-action = Pomniejsz
cmd-view-zoom_out-desc = Zmniejsz powiększenie
cmd-view-zoom_reset-action = Resetuj zoom
cmd-view-zoom_reset-desc = Resetuj do 100%
cmd-view-zoom_fit-action = Dopasuj do okna
cmd-view-zoom_fit-desc = Dopasuj zawartość do okna
cmd-view-fullscreen-action = Pełny ekran
cmd-view-fullscreen-desc = Przełącz pełny ekran

# Polecenia nawigacji
cmd-nav-back-action = Wstecz
cmd-nav-back-desc = Idź do poprzedniej lokalizacji
cmd-nav-forward-action = Dalej
cmd-nav-forward-desc = Idź do następnej lokalizacji
cmd-nav-up-action = Katalog nadrzędny
cmd-nav-up-desc = Przejdź poziom wyżej

# Polecenia okna
cmd-window-new-action = Nowe okno
cmd-window-new-desc = Otwórz nowe okno
cmd-window-close-action = Zamknij okno
cmd-window-close-desc = Zamknij aktualne okno

# Polecenia pomocy
cmd-help-show-action = Pomoc
cmd-help-show-desc = Pokaż skróty
cmd-help-about-action = O programie
cmd-help-about-desc = Pokaż O programie

# Polecenia ustawień
cmd-settings-open-action = Ustawienia
cmd-settings-open-desc = Otwórz ustawienia

# Polecenia dialogów
cmd-dialog-sauce-action = Info SAUCE
cmd-dialog-sauce-desc = Pokaż metadane SAUCE
cmd-dialog-export-action = Eksportuj
cmd-dialog-export-desc = Eksportuj jako obraz
cmd-dialog-filter-action = Filtruj
cmd-dialog-filter-desc = Przełącz filtr

# Polecenia odtwarzania
cmd-playback-toggle_scroll-action = Auto-przewijanie
cmd-playback-toggle_scroll-desc = Włącz/wyłącz przewijanie
cmd-playback-scroll_speed-action = Prędkość +
cmd-playback-scroll_speed-desc = Zwiększ prędkość
cmd-playback-scroll_speed_back-action = Prędkość -
cmd-playback-scroll_speed_back-desc = Zmniejsz prędkość
cmd-playback-baud_rate-action = Baud +
cmd-playback-baud_rate-desc = Zwiększ baud
cmd-playback-baud_rate_back-action = Baud -
cmd-playback-baud_rate_back-desc = Zmniejsz baud
cmd-playback-baud_rate_off-action = Baud wyłączony
cmd-playback-baud_rate_off-desc = Wyłącz emulację baud

# Polecenia zewnętrzne
cmd-external-command_0-action = Zewnętrzne 1
cmd-external-command_0-desc = Uruchom polecenie 1 (F5)
cmd-external-command_1-action = Zewnętrzne 2
cmd-external-command_1-desc = Uruchom polecenie 2 (F6)
cmd-external-command_2-action = Zewnętrzne 3
cmd-external-command_2-desc = Uruchom polecenie 3 (F7)
cmd-external-command_3-action = Zewnętrzne 4
cmd-external-command_3-desc = Uruchom polecenie 4 (F8)
error-loading-items=Błąd ładowania elementów
