...
impl Pipeline {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, format: wgpu::TextureFormat) -> Self {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(std::borrow::Cow::Borrowed(include_str!("../shaders/cubes.wgsl"))),
        });
        ...
        let depth_pipeline = DepthPipeline::new(device, format, depth_texture.create_view(&wgpu::TextureViewDescriptor::default()));
    ...
    fn update_depth_texture(&mut self, device: &wgpu::Device, size: Size<u32>) {
        if self.depth_texture_size.height != size.height || self.depth_texture_size.width != size.width {
    ...
    pub fn update(&mut self, device: &wgpu::Device, queue: &wgpu::Queue, target_size: Size<u32>, uniforms: &Uniforms, num_cubes: usize, cubes: &[cube::Raw]) {
    ...
    pub fn render(&self, target: &wgpu::TextureView, encoder: &mut wgpu::CommandEncoder, viewport: Rectangle<u32>, num_cubes: u32, show_depth: bool) {
        {
...
impl DepthPipeline {
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat, depth_texture: wgpu::TextureView) -> Self {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(std::borrow::Cow::Borrowed(include_str!("../shaders/depth.wgsl"))),
        });
    ...
    pub fn render(&self, encoder: &mut wgpu::CommandEncoder, target: &wgpu::TextureView, viewport: Rectangle<u32>) {
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
