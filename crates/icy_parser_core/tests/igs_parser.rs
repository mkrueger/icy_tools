...
fn test_igs_circle_command() {
    ...
    // G#K50,50,25:
    parser.parse(b"G#K50,50,25:", &mut sink);
...
fn test_igs_write_text() {
    ...
    assert_eq!(sink.igs_commands.len(), 1);
    match &sink.igs_commands[0] {
        IgsCommand::WriteText { x, y, justification, text } => {
            assert_eq!(*x, 10);
            assert_eq!(*y, 20);
            assert_eq!(*justification, 0);
...
fn test_igs_ellipse() {
    ...
    parser.parse(b"G#M100,100,50,30:", &mut sink);
...
fn test_igs_arc() {
    ...
    parser.parse(b"G#Q100,100,0,90,50:", &mut sink);
