...
pub enum PatternType {
    ...
    Hatch(u8),
    /// User defined fill (uses pattern index 0-7)
    UserDefined(u8),
...
impl PatternType {
    /// Get the fill pattern based on the pattern type, matching VDI st_fl_ptr behavior
    ///
    /// # Arguments
    /// * `user_pattern` - Array of user-defined patterns (up to 8 patterns, each 16 lines)
    ///
    /// # Returns
    /// A tuple of (pattern_data, pattern_mask) where:
    /// - pattern_data: the pattern array to use
    /// - pattern_mask: the mask value (0 for 1-line patterns, 7 for 8-line, 15 for 16-line)
    pub fn fill_pattern<'a>(&self, user_pattern: &'a [Vec<u16>; 8]) -> &'a [u16] {
        match self {
            ...
            PatternType::Pattern(index) => {
                ...
                if idx < 24 {
                    // All TYPE_PATTERN entries (dither + OEM patterns) - 8 lines each
                    &TYPE_PATTERN[idx]
                } else {
                    // Out of range, return solid
                    &SOLID_PATTERN
                }
            }
            PatternType::Hatch(index) => {
                let idx: usize = (*index as usize).saturating_sub(1);
                if idx < 6 {
                    // HATCH0 patterns (narrow hatches) - 8 lines each
                    &HATCH_PATTERN[idx]
                } else if idx < 12 {
                    // HATCH1 patterns (wide hatches) - 16 lines each
                    let wide_idx = idx - 6;
                    &HATCH_WIDE_PATTERN[wide_idx]
                } else {
                    // Out of range, return solid
                    &SOLID_PATTERN
                }
            ...
            PatternType::UserDefined(index) => {
                ...
                if idx < user_pattern.len() && !user_pattern[idx].is_empty() {
                    // User patterns are 16 lines (mask = 15)
                    &user_pattern[idx]
                } else {
                    // No user pattern available, return solid
                    &SOLID_PATTERN
                }
            }
            PatternType::Random => {
                // Use a pre-generated random pattern (16 lines)
                // In the original VDI this would generate new random data each time,
                // but for reproducibility we use a fixed pattern
                &RANDOM_PATTERN_FIXED
            }
            PatternType::StarTrek => &STAR_TREK,
        }
    }
}

// Fixed random pattern for reproducibility (16 lines)
pub const RANDOM_PATTERN_FIXED: [u16; 16] = [
    0x1c8c, 0x6987, 0x4b96, 0xbfbc, 0xaa0e, 0x1a66, 0x052b, 0xc73d, 0xf810, 0xad4e, 0xf44a, 0x49d3, 0x66c9, 0x0677, 0xadf1, 0x718a,
];
...
pub const STAR_TREK: [u16; 16] = [
    0x07E0, 0x0FF0, 0x1FD8, 0x1808, 0x1808, 0x1008, 0x1E78, 0x1348, 0x1108, 0x0810, 0x0B70, 0x0650, 0x07A0, 0x1E20, 0x1BC0, 0x1800,
];

pub const OEM_PATTERN: [[u16; 8]; 16] = [
    // Brick
