...
impl SettingsDialogState {
    pub fn modem_settings_content(&self) -> Element<'_, crate::ui::Message> {
        let modems = &self.temp_options.modems;
        ...
        for (idx, modem) in modems.iter().enumerate() {
            ...
            let modem_button = button(container(text(&modem.name).size(13)).width(Length::Fill).padding([6, 10]))
