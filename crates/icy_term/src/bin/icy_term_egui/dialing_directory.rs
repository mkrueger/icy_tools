...
impl DialingDirectory {
    ...
    pub fn show(&mut self, context: &egui::Context, connected: bool) -> Option<DialRequest> {
        ...
        let width = (screen.width() - 48.0).clamp(260.0, 1080.0);
        let height = (screen.height() - 64.0).clamp(160.0, 740.0);
        let narrow = width < 640.0;
        let mut request = None;
        let response = egui::Modal::new(egui::Id::new("dialing-directory"))
            ...
            .show(context, |ui| {
                ...
                egui::ScrollArea::vertical().id_salt("directory-overflow").max_height(height).show(ui, |ui| {
                    ...
                    if !editing {
                        ui.add_enabled_ui(!connected && self.confirmation.is_none(), |ui| {
                            let address_id = ui.make_persistent_id("quick-address");
                            let mut dial =
                                ui.is_enabled() && ui.memory(|memory| memory.has_focus(address_id)) && ui.input(|input| input.key_pressed(egui::Key::Enter));
                            ui.add(
                                egui::TextEdit::singleline(&mut self.quick_address)
                                    .id(address_id)
                                    .margin(egui::vec2(8.0, 6.0))
                                    .desired_width(f32::INFINITY)
                                    .hint_text(&*tr!("egui-quick-connect-hint")),
                            );
                            ui.horizontal_wrapped(|ui| {
                                ui.checkbox(&mut self.quick_utf8, "UTF-8");
                                dial |= ui
                                    .add_enabled(!self.quick_address.trim().is_empty(), egui::Button::new(&*tr!("egui-quick-connect")))
                                    .clicked();
                                let writable = self.phonebook.as_ref().is_some_and(|book| !book.book.write_lock);
                                if ui
                                    .add_enabled(
                                        writable && !self.quick_address.trim().is_empty(),
                                        egui::Button::new(&*tr!("egui-add-to-directory")),
                                    )
                                    .clicked()
                                {
                                    match icy_term::ConnectionInformation::parse(&self.quick_address) {
                                        Ok(info) if !info.host.is_empty() => {
                                            let book = self.phonebook.as_mut().unwrap();
                                            book.begin_new(false);
                                            let mut entry = Address::from(info);
                                            if matches!(entry.protocol, icy_net::ConnectionType::Websocket | icy_net::ConnectionType::SecureWebsocket) {
                                                entry.address = self.quick_address.clone();
                                            }
                                            if self.quick_utf8 {
                                        ...
                                        _ => self.error = Some("Enter a valid connection address".into()),
                                    }
                                }
                            });
                            if dial && !context.will_discard() {
                                match session::connection_config(&self.quick_address, self.quick_utf8) {
                                    Ok(_) => request = Some(DialRequest::Quick(self.quick_address.clone(), self.quick_utf8)),
                    ...
                    ui.allocate_ui_with_layout(egui::vec2(ui.available_width(), remaining), egui::Layout::top_down(egui::Align::Min), |ui| {
                        ...
                        if narrow {
                            let next = if self.details {
                                ...
                                self.list_ui(ui, connected, (ui.available_height() - 160.0).max(60.0))
                            };
                            if next.is_some() {
                                request = next;
                            }
                            ...
                            let list_width = (width * 0.36).clamp(240.0, 360.0);
                            ui.horizontal_top(|ui| {
                                ui.allocate_ui_with_layout(egui::vec2(list_width, body_height), egui::Layout::top_down(egui::Align::Min), |ui| {
                                    if let Some(entry) = self.list_ui(ui, connected, (body_height - 160.0).max(60.0)) {
                                        request = Some(entry);
                                    }
                                });
                                ui.separator();
        ...
        #[cfg(test)]
        {
            self.bounds = Some(response.response.rect);
        }
    ...
    fn list_ui(&mut self, ui: &mut egui::Ui, connected: bool, height: f32) -> Option<DialRequest> {
        ...
        ui.add_enabled_ui(!editing, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.add_enabled_ui(writable, |ui| {
                    if icon_button(ui, &self.icons.as_ref().unwrap().add, &*tr!("egui-new-entry")).clicked() {
                        self.phonebook.as_mut().unwrap().begin_new(false);
                        self.editor = Default::default();
                        self.details = true;
                        self.show_password = false;
                    }
                });
                reload = ui.add_enabled(self.source_result.is_none(), egui::Button::new(&*tr!("egui-reload"))).clicked();
                if self.source_result.is_some() {
                    ui.spinner();
                }
                if let Some(book) = &self.phonebook {
                    ui.label(format!("{} entries", book.filtered().len()));
                }
            });
            ...
            let search_enter = ui.memory(|memory| memory.has_focus(search_id)) && ui.input(|input| input.key_pressed(egui::Key::Enter));
            let search = ui.add(
                egui::TextEdit::singleline(&mut book.query)
                    .id(search_id)
                    .margin(egui::vec2(8.0, 6.0))
                    .hint_text(&*tr!("egui-directory-search"))
                    .desired_width(f32::INFINITY),
            );
            ...
            ui.horizontal_wrapped(|ui| {
                ui.selectable_value(&mut book.favorites_only, false, &*tr!("egui-all"));
                ui.selectable_value(&mut book.favorites_only, true, &*tr!("egui-favorites"));
                egui::ComboBox::from_id_salt("sort")
                    .selected_text(match book.sort {
                        SortOrder::Name => tr!("settings-modem-name"),
                        SortOrder::MostCalled => tr!("egui-most-called"),
                        SortOrder::LastCalled => tr!("egui-last-called"),
                    })
                    .show_ui(ui, |ui| {
            ...
            if !sources.is_empty() {
                egui::ComboBox::from_id_salt("source")
                    .selected_text(match book.source.as_deref() {
                        None => tr!("egui-all-sources"),
                        Some("") => tr!("egui-local-phonebook"),
            ...
            if (search.has_focus() || search_enter) && !editing && !indices.is_empty() {
                let position = book
                    .selected
                ...
                if down {
                    book.selected = Some(indices[(position + 1).min(indices.len() - 1)]);
                }
                if up {
                    book.selected = Some(indices[position.saturating_sub(1)]);
                }
                scroll_selection = down || up;
                if !connected && search_enter && !ui.ctx().will_discard() {
                        ...
                        ui.horizontal(|ui| {
                            let star = ui
                                .add_enabled(
                                    ...
                                    egui::Button::new(
                                        egui::RichText::new(if favorite { "\u{2605}" } else { "\u{2606}" })
                                            .size(18.0)
                                            .color(if favorite {
                                    ...
                                    .frame(false)
                                    .min_size(egui::vec2(26.0, 44.0)),
                                )
                                .on_hover_text(if favorite { tr!("egui-remove-favorite") } else { tr!("egui-add-favorite") });
                            if star.clicked() && !ui.ctx().will_discard() {
                                if let Err(error) = book.toggle_favorite(index) {
                                    self.error = Some(error);
                                }
                            }
                            let response = directory_row(ui, &name, &address, book.selected == Some(index));
                            if scroll_selection && book.selected == Some(index) {
    ...
    fn confirmation_ui(&mut self, ui: &mut egui::Ui) {
        let delete = self.confirmation == Some(Confirmation::Delete);
        ui.strong(if delete { tr!("egui-delete-question") } else { tr!("egui-discard-question") });
        if let Some(name) = self.phonebook.as_ref().and_then(|book| book.selection()).map(|entry| &entry.system_name) {
            ui.label(name);
        }
        ui.horizontal(|ui| {
            if ui.button(if delete { tr!("egui-delete") } else { tr!("egui-discard-close") }).clicked() && !ui.ctx().will_discard() {
                if delete {
                    if let Some(book) = &mut self.phonebook {
                        match book.delete() {
                            Ok(()) => {
...
fn directory_row(ui: &mut egui::Ui, name: &str, address: &str, selected: bool) -> egui::Response {
    ...
    child.add(
        egui::Label::new(egui::RichText::new(name).color(ui.visuals().text_color()))
            .selectable(false)
            .truncate(),
    );
    child.add(
        egui::Label::new(egui::RichText::new(address).small().color(ui.visuals().weak_text_color()))
            .selectable(false)
            .truncate(),
    );
...
mod tests {
    ...
    fn quick_connect_is_not_lost_when_the_directory_is_rendered() {
        ...
        let mut dialog = DialingDirectory {
            ...
            open: true,
            quick_address: "raw://localhost:2323".into(),
            ..Default::default()
        };
        let context = egui::Context::default();
        let request = click(&mut dialog, &context, &*tr!("egui-quick-connect")).unwrap();
        assert!(matches!(request, DialRequest::Quick(address, false) if address == "raw://localhost:2323"));
        assert!(!dialog.open);
    }
    ...
    fn quick_address_can_be_saved_as_a_profile() {
        ...
        let mut dialog = DialingDirectory {
            phonebook: Some(fixture.load()),
            open: true,
            quick_address: "ws://localhost:2323/terminal".into(),
            ..Default::default()
        };
        let context = egui::Context::default();
        click(&mut dialog, &context, &*tr!("egui-add-to-directory"));
        let book = dialog.phonebook.as_ref().unwrap();
