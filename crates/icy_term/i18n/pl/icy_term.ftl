connect-to = Połącz z { $address }...

settings-heading = Ustawienia
settings-enabled-checkbox = Włączone
settings-scaling = Skalowanie
settings-monitor-type = Monitor
settings-monitor-category = Monitor
settings-monitor-use-crt-filter-checkbox = Użyj filtra CRT
settings-monitor-brightness = Jasność
settings-monitor-contrast = Kontrast
settings-monitor-saturation = Nasycenie
settings-monitor-gamma = Gamma
settings-monitor-blur = Rozmycie
settings-monitor-curve = Krzywa
settings-monitor-scanlines = Linie skanowania
settings-iemsi-category = IEMSI
settings-iemsi-autologin-section = Login automatyczny IEMSI
settings-iemsi-alias = Alias
settings-iemsi-location = Lokalizacja
settings-iemsi-data-phone = Telefon danych
settings-iemsi-voice-phone = Telefon głosowy
settings-iemsi-birth-date = Data urodzenia

settings-monitor-color = Kolor
settings-monitor-grayscale = Skala szarości
settings-monitor-amber = Bursztyn
settings-monitor-green = Zielony
settings-monitor-apple2 = Apple ][
settings-monitor-futurystyczny = Futurystyczny
settings-scaling-nearest = Najbliższy
settings-scaling-linear = Liniowy

settings-terminal-category = Terminal
settings-terminal-general-section = Ogólne
settings-terminal-console-beep-checkbox = Beep konsoli
settings-terminal-open-settings-dir-button = Otwórz katalog ustawień
settings-terminal-dial-tone = Ton wybierania
settings-terminal-scrollback-lines = Scrollback
settings-terminal-scrollback-lines-unit = Linie

settings-keybinds-category = Skróty klawiszowe
settings-keybinds-disconnect = Rozłącz:
settings-keybinds-dialing-directory = Katalog wybierania:
settings-keybinds-send-login = Wyślij login:
settings-keybinds-capture-control = Kontrola przechwytywania:
settings-keybinds-show-settings = Pokaż ustawienia:
settings-keybinds-clear-screen = Wyczyść ekran:
settings-keybinds-toggle-fullscreen = Przełącz na pełny ekran:
settings-keybinds-upload = Prześlij:
settings-keybinds-download = Pobierz:
settings-keybinds-quit = Wyjdź:
settings-keybinds-show-find = Znaj

settings-modem-category = Modem
settings-modem-name = Nazwa

settings-paths-category = Ścieżki
settings-paths-header = Ścieżki systemowe
settings-paths-config-dir = Katalog konfiguracji
settings-paths-config-file = Plik konfiguracyjny
settings-paths-phonebook = Książka telefoniczna
settings-paths-log-file = Plik dziennika
settings-paths-open = Otwórz
settings-paths-editable-header = Ścieżki użytkownika
settings-paths-download-dir = Pobrane
settings-paths-capture-path = Ścieżka przechwytywania

settings-modem-device = Urządzenie
settings-modem-baud_rate = Baudrate
settings-modem-init_string = Ciąg inicjalizacyjny
settings-modem_dial_string = Ciąg wybierania
settings-modem-flow_control = Kontrola przepływu
settings-modem-nothing_selected = Brak wybranego modemu
settings-modem-list-section = Modemy
settings-modem-device-section = Urządzenie
settings-modem-serial-section = Port Szeregowy
settings-modem-commands-section = Polecenia Modemu
settings-modem-add-button = Dodaj
settings-modem-remove-button = Usuń
settings-modem-no-selection-hint = Dodaj modem lub wybierz jeden z listy
settings-modem-dial_prefix = Prefiks wybierania
settings-modem-format = Format
settings-modem-baud_rate-quick = Typowe
settings-modem-add-tooltip = Dodaj nowy modem
settings-modem-remove-tooltip = Usuń wybrany modem
settings-modem-init_command = Init
settings-modem-dial_suffix = Sufiks Wybierania
settings-modem-hangup_command = Rozłącz

settings-protocol-category = Protokoły
settings-protocol-list-section = Protokoły
settings-protocol-nothing-selected = Brak wybranego protokołu
settings-protocol-no-selection-hint = Dodaj protokół lub wybierz jeden z listy
settings-protocol-add-tooltip = Dodaj nowy protokół
settings-protocol-remove-tooltip = Usuń wybrany protokół
settings-protocol-move-up-tooltip = Przesuń protokół w górę
settings-protocol-move-down-tooltip = Przesuń protokół w dół
settings-protocol-general-section = Ogólne
settings-protocol-commands-section = Polecenia
settings-protocol-id = ID
settings-protocol-name = Nazwa
settings-protocol-description = Opis
settings-protocol-enabled = Włączony
settings-protocol-batch = Transfer wsadowy
settings-protocol-auto-transfer = Transfer automatyczny
settings-protocol-send-command = Polecenie wysyłania
settings-protocol-recv-command = Polecenie odbierania
settings-protocol-download-signature = Sygnatura pobierania
settings-protocol-upload-signature = Sygnatura wysyłania
settings-protocol-internal-hint = Protokół wewnętrzny - polecenia są opcjonalne

dialing_directory-connect-to = Szybkie połączenie z…
dialing_directory-new_bbs = Nowy BBS
dialing_directory-name-placeholder = Nazwa BBS
dialing_directory-filter-placeholder = Filtruj nazwy
dialing_directory-address = Adres
dialing_directory-phone-number = Numer Telefonu
dialing_directory-modem = Modem
dialing_directory-protocol = Protokół
dialing_directory-user = Użytkownik
dialing_directory-password = Hasło
dialing_directory-generate = Generu
dialing_directory-screen_mode = Tryb ekranu
dialing_directory-terminal_type = Typ terminala
dialing_directory-resolution = Kolumny×Wiersze
dialing_directory-resolution-info = {$width}×{$height}, {$cols} kol,
 {$colors} kolorów
dialing_directory-music-option = Muzyka Ansi
dialing_directory-baud-emulation = Emulacja baud
dialing_directory-autologin = Ciąg autologowania
dialing_directory-comment = Komentarz
dialing_directory-delete = Usuń wybrany BBS
dialing_directory-add = Dodaj nowy BBS
dialing_directory-settings = { settings-heading }
dialing_directory-connect-to-address = <Połącz z adresem>
dialing_directory-connect-button = Połącz
dialing_directory-not-called = Jeszcze nie dzwoniono
dialing_directory-last-call-date-format=Ostatnie połączenie było %m/%d/%Y o %H:%M
dialing_directory-created-at-date-format=Utworzono %m/%d/%Y o %H:%M
dialing_directory-date-format=%d/%m/%Y o %H:%M
dialing_directory-clear-filter = Wyczyść filtr
dialing_directory-starred-items = Filtruj oznaczone elementy
dialing_directory-no-entries = Brak pasujących wpisów
dialing_directory-add-bbs-button = Dodaj BBS
dialing_directory-version-warning = Ten katalog wybierania został utworzony w nowszej wersji IcyTerm. Proszę użyć nowszej wersji. Żadne zmiany nie zostaną zapisane.
dialing_directory-open_cache_button=Otwórz katalog pamięci podręcznej…
dialing_directory-generate-tooltip = Generuj losowe hasło
dialing_directory-generate-disabled-tooltip= 
    Hasło jest już ustawione,
    najpierw je usuń, aby wygenerować nowe
dialing_directory-invalid_modem = Nieprawidłowy modem
dialing_directory-no_modem_configured = Brak skonfigurowanego modemu
dialing_directory-select-modem = Wybierz modem
dialing_directory-login-settings = Ustawienia logowania
dialing_directory-notes = Notatki
dialing_directory-xep80-module = Moduł XEP80
dialing_directory-options = Options
dialing_directory-mouse-reporting = Enable mouse reporting

terminal-upload = Prześlij
terminal-download = Pobierz
terminal-hangup = Rozłącz
terminal-autologin = Wyślij nazwę użytkownika/hasło
terminal-dialing_directory = Katalog wybierania
terminal-menu = Menu
terminal-menu-copy = Kopiuj
terminal-menu-paste = Wklej
terminal-menu-export = Eksportuj bufor…
terminal-menu-info = Informacje o terminalu
toast-copied-to-clipboard = Skopiowano do schowka
terminal-find-hint = Znajdź tekst
terminal-find-no-results = Brak wyników
terminal-find-results = { $cur } z { $total }

protocol-select-download = Wybierz protokół pobierania
protocol-select-upload = Wybierz protokół przesyłania
file-dialog-save-download-as = Zapisz pobieranie jako
protocol-zmodem-description = Standardowy protokół
protocol-zmodem8k-description = 8k Zmodem
protocol-xmodem-description = Przestarzały protokół
protocol-xmodem1k-description = Rzadko używany
protocol-xmodem1kG-description = Czy to w ogóle istnieje?
protocol-ymodem-description = Ok, ale Zmodem jest lepszy
protocol-ymodemg-description = Szybka wersja Ymodem
protocol-text-description = Prześlij plik tekstowy

transfer-upload = { terminal-upload }
transfer-download = { terminal-download }
transfer-protocol = Protokół
transfer-checksize = Sprawdź/rozmiar:
transfer-elapsedtime = Upłynęło czasu
transfer-state = Stan:
transfer-file = Plik:
transfer-rate = Prędkość transferu
transfer-bps = { $bps } na sekundę
transfer-status-active = Aktywny
transfer-status-complete = Zakończono
transfer-log-all = Wszystkie
transfer-log-warnings = Ostrzeżenia
transfer-log-errors = Błędy
transfer-waiting = Oczekiwanie na rozpoczęcie transferu...

menu-item-discuss=Forum
menu-item-report-bug=Zgłoś błąd
menu-item-check-releases=Nowe wydania
menu-item-settings={ settings-heading }
menu-item-capture-dialog=Przechwyć…
menu-upgrade_version=Uaktualnij do { $version }

toolbar-stop-capture=Zatrzymaj przechwytywanie
toolbar-show-iemsi=Pokaż IEMSI
toolbar-hide-iemsi=Ukryj IEMSI
toolbar-stop-playing1 = Przestań proszę
toolbar-stop-playing2 = Moje uszy krwawią
toolbar-stop-playing3 = Przestań tę szaleństwo
toolbar-stop-playing4 = Błagam cię
toolbar-stop-playing5 = Litość
toolbar-stop-playing6 = Wystarczy

capture-dialog-capture-folder = Katalog
capture-dialog-capture-file = Plik
capture-dialog-capture-title = Przechwytywanie
capture-dialog-capture-button = Start
capture-dialog-dir-not-exist = Katalog nie istnieje
capture-dialog-not-directory = Ścieżka nie jest katalogiem
capture-dialog-file-exists = Plik istnieje i zostanie nadpisany

show-iemsi-dialog-heading=Informacje o serwerze IEMSI
show-iemsi-dialog-name=Nazwa
show-iemsi-dialog-location=Lokalizacja
show-iemsi-dialog-operator=Sysop
show-iemsi-dialog-notice=Powiadomienie
show-iemsi-dialog-capabilities=Możliwości
show-iemsi-dialog-id=Id

delete-bbs-title=Usuń BBS
delete-bbs-question=Czy na pewno chcesz usunąć '{ $system }'?
delete-bbs-delete-button=Usuń  

select-bps-dialog-heading = Wybierz emulację BPS
select-bps-dialog-bps-max = MAKS BPS
select-bps-dialog-bps = { $bps } BPS
select-bps-dialog-bps-custom = Niestandardowy

show-disconnected-heading=BRAK NOŚNIKA
show-disconnected-message=Rozłączono z { $system } o { $time }

error-address-book-load-title=BŁĄD W ŁADOWANIU KSIĄŻKI ADRESOWEJ
error-address-book-load-secondary=Plik książki adresowej wydaje się uszkodzony. IcyTerm uruchomi się z pustą listą; uszkodzony plik NIE zostanie nadpisany.

    Sugestie:
     • Wykonaj kopię zapasową i usuń/zmień nazwę istniejącego pliku.
     • Uruchom ponownie IcyTerm aby utworzyć nowy.
     • Jeśli masz kopię, przywróć ją przed restartem.

    Szczegóły techniczne:

error-script-title=Błąd Skryptu
error-script-execution-failed=Wykonanie skryptu nie powiodło się
error-script-file-read-failed=Nie udało się odczytać pliku skryptu

script-dialog-title=Wybierz Skrypt Lua
script-dialog-filter-lua=Skrypty Lua
script-dialog-filter-all=Wszystkie Pliki

cli-error-url-parse=Błąd podczas analizowania URL '{ $url }': { $error }
cli-error-script-not-found=Nie można odczytać '{ $file }': Plik nie istnieje
cli-error-script-not-file=Nie można odczytać '{ $file }': To jest katalog

# Pomoc wiersza poleceń
app-about=Emulator terminala do łączenia się z systemami tablic ogłoszeń (BBS) z pełną obsługą CP437, PETSCII i ViewData
arg-url-help=Połącz się z BBS przy użyciu podanego URL
arg-url-format=Format URL: [(telnet|ssh|raw)://][użytkownik[:hasło]@]domena[:port]
arg-run-help=Uruchom skrypt Lua po starcie
arg-mcp-port-help=Włącz serwer MCP na podanym porcie

# Help dialog
help-title = Skróty klawiszowe
help-subtitle = Szybka ściągawka dla iCY TERM

# Kategorie
help-category-connection = Połączenie
help-category-authentication = Uwierzytelnianie
help-category-file-transfer = Transfer plików
help-category-windows = Okna
help-category-display = Wyświetlanie
help-category-tools = Narzędzia
help-category-editing = Edycja

# Dodatkowe akcje
help-action-switch-window = Przełącz okno
help-action-smart-paste = Inteligentne wklejanie
help-key-middle-click = Środkowy przycisk

# Opisy
help-desc-dialing-directory = Otwórz katalog wybierania BBS
help-desc-disconnect = Rozłącz bieżące połączenie
help-desc-exit = Zakończ działanie iCY TERM
help-desc-auto-login = Wyślij nazwę użytkownika i hasło
help-desc-send-username = Wyślij tylko nazwę użytkownika
help-desc-send-password = Wyślij tylko hasło
help-desc-upload = Prześlij pliki do BBS
help-desc-download = Pobierz pliki z BBS
help-desc-close-window = Zamknij bieżące okno
help-desc-new-window = Otwórz nowe okno terminala
help-desc-switch-window = Skocz do okna 1–10
help-desc-fullscreen = Przełącz tryb pełnego ekranu
help-desc-clear-screen = Wyczyść zawartość ekranu
help-desc-capture-screen = Zapisz ekran jako obraz
help-desc-capture-session = Rozpocznij/Zatrzymaj przechwytywanie tekstu
help-desc-find = Szukaj w buforze
help-desc-settings = Otwórz ustawienia
help-desc-about = Pokaż informacje o iCY TERM
help-desc-help = Pokaż ten dialog
help-desc-copy = Skopiuj zaznaczenie
help-desc-paste = Wklej ze schowka
help-desc-smart-paste = Wklej albo (jeśli jest zaznaczenie) najpierw skopiuj

# Etykiety akcji (tam gdzie można, używamy istniejących kluczy)
help-action-dialing-directory = { terminal-dialing_directory }
help-action-disconnect = Rozłącz
help-action-exit = Wyjdź
help-action-auto-login = Auto‑login
help-action-send-username = Wyślij nazwę użytkownika
help-action-send-password = Wyślij hasło
help-action-close-window = Zamknij okno
help-action-new-window = Nowe okno
help-action-fullscreen = Pełny ekran
help-action-clear-screen = Wyczyść ekran
help-action-scrollback = Pokaż historię
help-action-capture-screen = Przechwyć ekran
help-action-capture-session = Przechwyć sesję
help-action-find = Szukaj
help-action-run-script = Uruchom skrypt
help-action-about = O programie
help-action-help = Pomoc

help-desc-run-script = Uruchom skrypt Lua
help-action-open-serial = Port Szeregowy
help-desc-open-serial = Otwórz połączenie portu szeregowego

# Zoom
help-category-zoom = Powiększenie
help-action-zoom-in = Powiększ
help-action-zoom-out = Pomniejsz
help-action-zoom-reset = Resetuj powiększenie
help-action-zoom-auto = Dopasuj do okna
help-desc-zoom-in = Zwiększ poziom powiększenia
help-desc-zoom-out = Zmniejsz poziom powiększenia
help-desc-zoom-reset = Resetuj do 100%
help-desc-zoom-auto = Dopasuj zawartość do okna

# Open Serial Dialog
open-serial-dialog-title = Otwórz Port Szeregowy
open-serial-dialog-device = Urządzenie
open-serial-dialog-baud-rate = Szybkość transmisji
open-serial-dialog-format = Format
open-serial-dialog-flow-control = Kontrola przepływu
open-serial-dialog-connect = Połącz
open-serial-dialog-auto-detect = Auto wykryj

# Terminal Info Dialog
terminal-info-dialog-screen-section = Ekran
terminal-info-dialog-resolution = Rozmiar
terminal-info-dialog-font-size = Rozmiar Czcionki
terminal-info-dialog-caret-section = Kursor
terminal-info-dialog-caret-shape = Kształt
terminal-info-dialog-caret-position = Pozycja
terminal-info-dialog-caret-visible = Widoczny
terminal-info-dialog-caret-blinking = Migający
terminal-info-dialog-input-mode = Tryb Wprowadzania
terminal-info-dialog-terminal-section = Terminal
terminal-info-dialog-auto-wrap = Automatyczne Zawijanie
terminal-info-dialog-scroll-mode = Tryb Przewijania
terminal-info-dialog-margins = Marginesy
terminal-info-dialog-mouse-tracking = Śledzenie Myszy
terminal-info-dialog-inverse-colors = Kolory Odwrócone
terminal-info-dialog-use-ice-colors = Ice Colors
terminal-info-dialog-yes = Tak
terminal-info-dialog-no = Nie
terminal-info-dialog-not-set = Nie ustawiono
terminal-info-dialog-copy-button = Kopiuj
terminal-info-dialog-copied = Skopiowano!
terminal-info-dialog-mouse-mode-tooltip-off = Śledzenie myszy wyłączone
terminal-info-dialog-mouse-mode-tooltip-x10 = Tryb X10 - tylko zdarzenia kliknięcia
terminal-info-dialog-mouse-mode-tooltip-vt200 = Tryb VT200 - zdarzenia kliknięcia i zwolnienia
terminal-info-dialog-mouse-mode-tooltip-vt200highlight = Tryb VT200 highlight - z podświetleniem tekstu
terminal-info-dialog-mouse-mode-tooltip-btnevent = Śledzenie przycisków - w tym ruch podczas naciśnięcia
terminal-info-dialog-mouse-mode-tooltip-anyevent = Pełne śledzenie - wszystkie zdarzenia raportowane
terminal-info-dialog-shape-tooltip-block = Pełny kursor blokowy
terminal-info-dialog-shape-tooltip-underline = Cienka linia na dole
terminal-info-dialog-shape-tooltip-bar = Pionowa kreska między znakami
terminal-info-dialog-apply-button = Zastosuj
