...
/// Default EGA 16-color palette (8-bit RGB values).
const DEFAULT_EGA_PALETTE: [[u8; 3]; 16] = [
...
/// Parse a Moebius palette from JSON into a fixed 16-color array.
/// Falls back to default EGA palette if parsing fails or palette is incomplete.
pub fn parse_moebius_palette(palette_json: &serde_json::Value) -> [[u8; 3]; 16] {
    fn clamp_u64_to_u8(v: u64, max: u8) -> u8 {
        let max_u64 = max as u64;
        (v.min(max_u64)) as u8
    }

    // Moebius internally stores EGA/C64 palette components as 6-bit (0-63) and
    // expands them for display. On the wire we may see either 0-63 or 0-255.
    fn convert_6bits_to_8bits(value: u8) -> u8 {
        (value << 2) | ((value & 0x30) >> 4)
    }

    let mut palette = DEFAULT_EGA_PALETTE;
    let mut max_component: u8 = 0;

    if let serde_json::Value::Array(arr) = palette_json {
        for (i, entry) in arr.iter().take(16).enumerate() {
            if let serde_json::Value::Object(obj) = entry {
                let r = obj.get("r").and_then(|v| v.as_u64()).unwrap_or(0);
                let g = obj.get("g").and_then(|v| v.as_u64()).unwrap_or(0);
                let b = obj.get("b").and_then(|v| v.as_u64()).unwrap_or(0);

                // Temporarily clamp to 0..=255; we may rescale later.
                let rr = clamp_u64_to_u8(r, 255);
                let gg = clamp_u64_to_u8(g, 255);
                let bb = clamp_u64_to_u8(b, 255);
                max_component = max_component.max(rr).max(gg).max(bb);
                palette[i] = [rr, gg, bb];
            }
        }
    }

    // If palette looks like 6-bit values (<=63), expand to 8-bit.
    if max_component <= 63 {
        for rgb in &mut palette {
            rgb[0] = convert_6bits_to_8bits(rgb[0]);
            rgb[1] = convert_6bits_to_8bits(rgb[1]);
            rgb[2] = convert_6bits_to_8bits(rgb[2]);
        }
    }
...
pub struct ConnectedDocument {
    ...
    /// Font name
    pub font: String,
    /// Color palette (16 RGB colors, each as [r, g, b])
    pub palette: [[u8; 3]; 16],
    /// SAUCE title
    pub title: String,
    /// SAUCE author
...
impl MoebiusDoc {
    ...
    pub fn into_connected_document(self, user_id: u32, users: Vec<User>) -> Result<ConnectedDocument, super::compression::CompressionError> {
        ...
        // Parse palette from JSON array
        let palette = parse_moebius_palette(&self.palette);
