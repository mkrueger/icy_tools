settings-heading = Settings
settings-scaling = Scaling
settings-monitor-type = Monitor
settings-restore-defaults-button = Restore Defaults
settings-monitor-category = Monitor
settings-monitor-use-crt-filter-checkbox = Use CRT filter
settings-monitor-brightness = Brightness
settings-monitor-contrast = Contrast
settings-monitor-saturation = Saturation
settings-monitor-gamma = Gamma
settings-monitor-blur = Blur
settings-monitor-curve = Curve
settings-monitor-scanlines = Scanlines
settings-background_color-label=Border color:
settings-theme = Theme

settings-monitor-color = Color
settings-monitor-grayscale = Grayscale
settings-monitor-amber = Amber
settings-monitor-green = Green
settings-monitor-apple2 = Apple ][
settings-monitor-futuristic = Futuristic
settings-monitor-custom = Custom

settings-enabled-checkbox = Enabled
settings-appearance-section = Appearance
settings-scaling-mode-label = Auto scaling
settings-integer-scaling-label = Use integer scaling
settings-bilinear-filtering-label = Bilinear Filtering
settings-color-tone-section = Color & Tone
settings-bloom-glow-section = Bloom & Glow
settings-bloom-threshold-label = Threshold
settings-bloom-radius-label = Radius
settings-glow-strength-label = Glow Strength
settings-phosphor-persistence-label = Phosphor Persistence
settings-scanlines-section = Scanlines
settings-scanline-thickness-label = Thickness
settings-scanline-sharpness-label = Sharpness
settings-scanline-phase-label = Phase
settings-geometry-section = Geometry
settings-curvature-x-label = Curvature X
settings-curvature-y-label = Curvature Y
settings-noise-artifacts-section = Noise & Artifacts
settings-noise-level-label = Noise Level
settings-sync-wobble-label = Sync Wobble
# Dialog buttons
dialog-ok-button = OK
dialog-cancel-button = Cancel
dialog-yes-button = Yes
dialog-no-button = No
dialog-close-button = Close
dialog-delete-button = Delete
dialog-overwrite-button = Overwrite

# Export dialog
export-dialog-title = Export Buffer
export-dialog-export-button = Export
export-dialog-folder = Folder:
export-dialog-file = File:
export-dialog-utf8-output = UTF-8 output
export-dialog-dir-not-exist = Directory does not exist!
export-dialog-not-directory = Path is not a directory!

# ═══════════════════════════════════════════════════════════════════════════════
# Common Command System Keys (shared between icy_view and icy_term)
# ═══════════════════════════════════════════════════════════════════════════════

# Categories
cmd-category-file = File
cmd-category-edit = Edit
cmd-category-view = View
cmd-category-navigation = Navigation
cmd-category-window = Window
cmd-category-help = Help
cmd-category-settings = Settings
cmd-category-general = General

# Additional categories for icy_term and other apps
cmd-category-connection = Connection
cmd-category-login = Authentication
cmd-category-transfer = File Transfer
cmd-category-terminal = Terminal
cmd-category-capture = Capture
cmd-category-script = Scripting
cmd-category-app = Application

# Additional categories for icy_view
cmd-category-playback = Playback
cmd-category-dialog = Dialogs
cmd-category-external = External Tools

# File commands
cmd-file-new-action = New
cmd-file-new-desc = Create a new file
cmd-file-new-menu = New…
cmd-file-open-action = Open
cmd-file-open-desc = Open a file
cmd-file-open-menu = Open…
cmd-file-save-action = Save
cmd-file-save-desc = Save the file
cmd-file-save-menu = Save
cmd-file-save_as-action = Save As
cmd-file-save_as-desc = Save file with a new name
cmd-file-save_as-menu = Save As…
cmd-file-export-action = Export
cmd-file-export-desc = Export file
cmd-file-export-menu = Export…
cmd-file-close-action = Close
cmd-file-close-desc = Close current file
cmd-file-close-menu = Close

# Edit commands
cmd-edit-undo-action = Undo
cmd-edit-undo-desc = Undo last action
cmd-edit-undo-menu = Undo
cmd-edit-redo-action = Redo
cmd-edit-redo-desc = Redo last undone action
cmd-edit-redo-menu = Redo
cmd-edit-cut-action = Cut
cmd-edit-cut-desc = Cut selection to clipboard
cmd-edit-cut-menu = Cut
cmd-edit-copy-action = Copy
cmd-edit-copy-desc = Copy selection to clipboard
cmd-edit-copy-menu = Copy
cmd-edit-paste-action = Paste
cmd-edit-paste-desc = Paste from clipboard
cmd-edit-paste-menu = Paste
cmd-edit-delete-action = Delete
cmd-edit-delete-desc = Delete selection
cmd-edit-delete-menu = Delete
cmd-edit-select_all-action = Select All
cmd-edit-select_all-desc = Select all content
cmd-edit-select_all-menu = Select All

# View commands
cmd-view-zoom_in-action = Zoom In
cmd-view-zoom_in-desc = Increase zoom level
cmd-view-zoom_in-menu = Zoom In
cmd-view-zoom_out-action = Zoom Out
cmd-view-zoom_out-desc = Decrease zoom level
cmd-view-zoom_out-menu = Zoom Out
cmd-view-zoom_reset-action = Reset Zoom
cmd-view-zoom_reset-desc = Reset to 100% zoom
cmd-view-zoom_reset-menu = Reset Zoom
cmd-view-zoom_fit-action = Fit to Window
cmd-view-zoom_fit-desc = Auto-fit content to window
cmd-view-zoom_fit-menu = Fit to Window
cmd-view-fullscreen-action = Fullscreen
cmd-view-fullscreen-desc = Toggle fullscreen mode
cmd-view-fullscreen-menu = Fullscreen

# Navigation commands
cmd-nav-back-action = Go Back
cmd-nav-back-desc = Navigate to previous location
cmd-nav-back-menu = Back
cmd-nav-forward-action = Go Forward
cmd-nav-forward-desc = Navigate to next location
cmd-nav-forward-menu = Forward
cmd-nav-up-action = Parent Directory
cmd-nav-up-desc = Go up one directory level
cmd-nav-up-menu = Up

# Window commands
cmd-window-new-action = New Window
cmd-window-new-desc = Open a new window
cmd-window-new-menu = New Window
cmd-window-close-action = Close Window
cmd-window-close-desc = Close current window
cmd-window-close-menu = Close Window

# Help commands
cmd-help-show-action = Help
cmd-help-show-desc = Show keyboard shortcuts
cmd-help-show-menu = Keyboard Shortcuts
cmd-help-about-action = About
cmd-help-about-desc = Show about dialog
cmd-help-about-menu = About

# Settings commands
cmd-settings-open-action = Settings
cmd-settings-open-desc = Open settings dialog
cmd-settings-open-menu = Settings…

# Focus navigation commands
cmd-focus-next-action = Focus Next
cmd-focus-next-desc = Move focus to next element
cmd-focus-next-menu = Focus Next
cmd-focus-previous-action = Focus Previous
cmd-focus-previous-desc = Move focus to previous element
cmd-focus-previous-menu = Focus Previous
