...
impl ShaderSegmentedControl {
    ...
    fn view_multi_select_internal<T: Clone + PartialEq + Send + 'static>(
        ...
        // Collect Char segments up-front (before moving `segments` into the Canvas program)
        let char_segments: Vec<(usize, char)> = segments
            .iter()
            .enumerate()
            .filter_map(|(idx, seg)| match seg.content {
                SegmentContent::Char(ch) => Some((idx, ch)),
                _ => None,
            })
            .collect();

        // Clone shared inputs for the GPU char overlay
        let font_for_char = font.clone();
        let char_colors_for_char = char_colors.clone();

        // Calculate segment widths
        let segment_widths: Vec<f32> = segments.iter().map(|seg| calculate_segment_width(seg, &font)).collect();
        let segment_widths_for_char = segment_widths.clone();
        ...
        // For multi-select, we use a special overlay that highlights based on selected_mask
        let text_overlay: Element<'_, SegmentedControlMessage<T>> = widget::canvas(MultiSelectTextOverlayProgram::<T> {
            segments,
            segment_widths,
            selected_mask,
            font,
        ...
        // Only create char overlay if there are Char segments
        // This avoids pipeline state conflicts when multiple controls share the same Pipeline
        if char_segments.is_empty() {
            // No char segments - just stack shader + text overlay
            widget::stack![shader_bg, text_overlay]
                .width(Length::Fixed(total_width))
                .height(Length::Fixed(total_height))
                .into()
        } else {
            let char_overlay: Element<'_, SegmentedControlMessage<T>> = widget::shader(SegmentedCharGlyphProgram {
                char_segments,
                segment_widths: segment_widths_for_char,
                selected_index: first_selected_index,
                font: font_for_char,
                char_colors: char_colors_for_char,
            })
            .width(Length::Fixed(total_width))
            .height(Length::Fixed(total_height))
            .into();
            ...
            widget::stack![shader_bg, text_overlay, char_overlay]
                .width(Length::Fixed(total_width))
                .height(Length::Fixed(total_height))
                .into()
        }
