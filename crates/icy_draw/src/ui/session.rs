...
impl SessionManager {
    ...
    /// Load autosave data
    pub fn load_autosave(&self, autosave_path: &PathBuf) -> Option<Vec<u8>> {
        if autosave_path.exists() { fs::read(autosave_path).ok() } else { None }
    }
    ...
    /// Get the autosave delay
    pub fn autosave_delay_secs(&self) -> u64 {
        self.autosave_delay_secs
    }

    /// Clean up orphaned autosave files
    /// Call this after session restore to remove autosaves that are no longer needed
    pub fn cleanup_orphaned_autosaves(&self, valid_paths: &[PathBuf]) {
        if let Ok(entries) = fs::read_dir(&self.session_dir) {
            for entry in entries.flatten() {
                let path = entry.path();
                if path.extension().map_or(false, |e| e == "autosave") {
                    if !valid_paths.contains(&path) {
                        let _ = fs::remove_file(&path);
                        log::debug!("Removed orphaned autosave: {:?}", path);
                    }
                }
            }
        }
...
/// Convert string back to EditMode
pub fn string_to_edit_mode(s: &str) -> EditMode {
    match s.to_lowercase().as_str() {
        "bitfont" => EditMode::BitFont,
        "charfont" => EditMode::CharFont,
        "animation" => EditMode::Animation,
        _ => EditMode::Ansi,
    }
}
