...
impl TextureRenderer {
    ...
    pub fn render_to_buffer(&self, gl: &glow::Context, input_texture: glow::Texture, render_buffer_size: Vec2, options: &TerminalOptions) -> (Vec2, Vec<u8>) {
        unsafe {
            gl.disable(glow::SCISSOR_TEST);

            let monitor_settings = &options.monitor_settings;
            let framebuffer: glow::NativeFramebuffer = gl.create_framebuffer().unwrap();
    ...
    fn create_output_texture(&self, gl: &glow::Context, render_buffer_size: Vec2) -> glow::Texture {
        unsafe {
